...
export const projects = [
...

    {
        ...
        challenges:
            'Learning to integrate AI with interview preparation workflows, implementing a seamless user interface for recording and managing interviews, and navigating cloud deployment complexities to ensure scalability and reliability. Also setting up CI/CD pipeline.',
          
        whatILearned: "Gained experience integrating AI for practical applications, designing robust backends using .NET and PostgreSQL, and building modern, responsive user interfaces with Next.js and Tailwind CSS. Additionally, I deepened my understanding of containerization with Docker and cloud deployment using AWS. I used Docker to containerize the application, making deployment more consistent and manageable across different environments. I also set up and configured AWS EC2 for hosting the backend, utilized RDS for database management, and stored media files in S3 for efficient access and scalability. Managing deployments, configuring Docker containers, and ensuring smooth service operation on AWS expanded my cloud knowledge, particularly in handling server configurations, storage management, and cost-effective resource allocation. To streamline deployments, I implemented a CI/CD pipeline using GitHub Actions. Whenever I pushed changes to GitHub, the pipeline would build the application, push the updated image to Docker Hub, and send a webhook request to my server. The server would then pull the new image from Docker Hub and restart the container with the latest version, ensuring that updates were deployed automatically with minimal manual intervention.",
        details: "The application customizes interview preparation by generating questions based on user-uploaded resumes and job descriptions. Users can record their answers and receive AI-generated feedback, making the experience more personalized and data-driven. The backend, built with .NET and PostgreSQL, ensures efficient data handling and scalability, while the frontend provides an intuitive and seamless user experience with Next.js and Tailwind CSS. The entire system is containerized with Docker, allowing for consistent deployment and easier maintenance. To automate deployments, I built a CI/CD pipeline using GitHub Actions. When changes were pushed to GitHub, the pipeline automatically built the application, pushed the new Docker image to Docker Hub, and triggered a webhook on my server. The server then pulled the updated image and restarted the container, ensuring smooth and efficient deployments. The application is hosted on AWS, with EC2 handling the backend, RDS managing the database, and S3 storing recorded responses. Since I am using AWS’s free tier, the application may occasionally be down if resource limits are exceeded. I may have also removed the link to avoid exceeding free tier limitations. If the link is unavailable, please refer to the provided video for a demo of the application.",
